...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.current_threads = 6
        self.current_used_files: List[Path] = []
    ...
    @staticmethod
    def _bmp_support_reason(path: Path) -> Tuple[bool, str]:
        try:
            ...
            if header[0:2] != b"BM":
                return False, "not a BMP signature"
            ...
            if bits_per_pixel == 8:
                if compression == 0:
                    return True, ""
                return False, f"8-bit compression={compression} (expected 0)"
            if bits_per_pixel == 24:
                if compression == 0:
                    return True, ""
                return False, f"24-bit compression={compression} (expected 0)"
            if bits_per_pixel == 32:
                if compression in (0, 3, 6):
                    return True, ""
                return False, f"32-bit compression={compression} (expected 0, 3 or 6)"
            return False, f"unsupported bit depth: {bits_per_pixel}"
        except OSError:
    ...
    def _validate_inputs(self) -> List[Path]:
        ...
        for path in self.selected_files:
            is_valid, reason = self._bmp_support_reason(path)
            if is_valid:
                valid.append(path)
            else:
        ...
        if not valid:
            raise ValueError(
                "Need at least 1 valid BMP file (8/24-bit uncompressed or 32-bit BI_RGB/BI_BITFIELDS/BI_ALPHABITFIELDS)."
            )

        if invalid:
            self._log("Skipped unsupported BMP files:")
    ...
    def _run_filters(self) -> None:
        ...
        if hasattr(self, "threadsSpinBox"):
            threads = int(self.threadsSpinBox.value())
        elif hasattr(self, "threadsLineEdit"):
            try:
                threads = int(self.threadsLineEdit.text().strip())
            except ValueError:
                threads = 6
        if threads < 1:
            threads = 1
        ...
        self.current_used_files = chosen[:]
        self._log("=" * 60)
        self._log(f"Running: {program} {' '.join(args)}")
        self._log(f"Selected files: {len(self.selected_files)} | Valid for run: {len(chosen)}")
    ...
    def _on_finished(self, exit_code: int, _exit_status) -> None:
        ...
        report_path = self._write_report(out_dir, self.current_used_files, threads, exit_code, stdout_text, stderr_text)
