...
class ImageAnalyzerGUI:
    ...
    def _build_ui(self) -> None:
        ...
        subtitle = Label(
            title_frame,
            text=(
                "Drop BMP files or use Add Files. Supports 8/24-bit uncompressed and "
                "32-bit BI_RGB/BI_BITFIELDS/BI_ALPHABITFIELDS BMP."
            ),
            font=("Arial", 10),
            bg="#f0f0f0",
            fg="#555",
    ...
    def _add_files(self, files: list[str]) -> None:
        ...
        for file_path in files:
            if len(self.selected_files) >= MAX_FILES:
                self._append_log(f"Reached maximum of {MAX_FILES} files.")
                break
    ...
    @staticmethod
    def _is_supported_bmp(path: Path) -> bool:
        try:
            ...
            if bits_per_pixel == 32:
                return compression in (0, 3, 6)
            return False
        except OSError:
            return False

    def _validate_inputs(self) -> list[Path]:
        valid = [p for p in self.selected_files if self._is_supported_bmp(p)]
        if not valid:
            raise ValueError(
                "Need at least 1 supported BMP file (8/24-bit uncompressed or 32-bit BI_RGB/BI_BITFIELDS/BI_ALPHABITFIELDS).\n"
