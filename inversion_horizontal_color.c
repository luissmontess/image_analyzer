...
void inversion_horizontal_color(const char* input_path, const char* name_output) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            output_rows[y][dst_index + 0] = input_rows[y][src_index + 0];
            output_rows[y][dst_index + 1] = input_rows[y][src_index + 1];
            output_rows[y][dst_index + 2] = input_rows[y][src_index + 2];
        }
    ...
    if (outputLog != NULL) {
        fprintf(outputLog, "Función: inversion_horizontal_color, con %s\n", input_path);
