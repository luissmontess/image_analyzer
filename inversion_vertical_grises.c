...
void inversion_vertical_escala_grises(const char* input_path, const char* name_output) {
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x++) {
            ...
            unsigned char blue = input_rows[src_y][index + 0];
            unsigned char green = input_rows[src_y][index + 1];
            unsigned char red = input_rows[src_y][index + 2];
        ...
        for (int p = width * 3; p < row_padded; p++) {
            output_rows[y][p] = input_rows[src_y][p];
        }
    }
    ...
    if (outputLog != NULL) {
        fprintf(outputLog, "Función: inversion_vertical_escala_grises, con %s\n", input_path);
