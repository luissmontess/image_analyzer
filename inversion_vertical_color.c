...
void inversion_vertical_color(const char* input_path, const char* name_output) {
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x++) {
            int index = x * 3;
            output_rows[y][index + 0] = input_rows[src_y][index + 0];
            output_rows[y][index + 1] = input_rows[src_y][index + 1];
            output_rows[y][index + 2] = input_rows[src_y][index + 2];
        }
    ...
    if (outputLog != NULL) {
        fprintf(outputLog, "Función: inversion_vertical_color, con %s\n", input_path);
